...
2. Backend abstraction — swap from env vars to OpenBao without changing agent code
...
CredentialProvider (Presidium)
    ...
    ↓ resolves value from backend (env/file/OpenBao/AWS)
...
class VaultCredentialProvider:
    """Reads credentials from OpenBao (Vault-compatible) KV v2 engine.
...
presidium:
  credentials:
    ...
    # OpenBao backend config (presidium-contrib[openbao])
...
| C2 | Backend abstraction | Protocol with env/file defaults | OpenBao-only, env-only | Same interface-first pattern as every other Presidium component |
...
2. **Dynamic secrets**: should Presidium support OpenBao.s dynamic secret engines (database, AWS)? (M3)

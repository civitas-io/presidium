...
| P8 | Grant.condition enforcement | **Real gap closed, 2026-08-25**: this design doc's own "Grant Integration" section above always specified real per-grant condition evaluation -- but `CelPolicyEngine`'s implementation never actually compiled/evaluated `condition`, only serialized it into the activation as an inert string. Found and reported externally (a downstream consumer's own audit of this codebase), not caught internally first. Fixed to match the design exactly: `_build_activation()` now compiles (cached by expression text) and evaluates each grant's `condition` against a minimal `agent`/`request`/`time` activation, excluding the grant from `active_grants` on a falsy result, a compile error, or a raise -- fail-closed, matching P3's existing invariant, not a new one. | Update the docstring to say it's unenforced (option (b) from the external report) | The field's own documented purpose (per-grant fine-grained authorization) is only real if it's real -- (b) would have papered over a genuine security gap rather than closing it, and the design was already correct; only the implementation was missing. |
